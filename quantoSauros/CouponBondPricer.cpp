#include "CouponBondPricer.h"

namespace quantoSauros {

	void CouponBondPricer::dividePeriods(){
	...
	void CouponBondPricer::generatePaths(){
		...
		for (int simIndex = 0; simIndex < m_simulationNum; simIndex++){
			for (int periodIndex = 0; periodIndex < m_periodNum; periodIndex++){			
				...
				//Fixed Coupon Bond
				m_data[simIndex][periodIndex] = FixedCouponBondData(
					m_today, m_args.getMaturityDate(), period, 
					startTenor, timeGrid, m_args.getDayCounter(),
	...
	void CouponBondPricer::calculatePrice(){
